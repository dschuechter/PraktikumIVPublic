...
p_initial = [0.04,0.005,1,0.07,0.005,0.2,0]
popt, pcov = curve_fit(gauss, x, y, p0=p_initial)
x_fit = np.linspace(x[0],x[-1],num=1000)
y_fit = gauss(x_fit, *popt)

plt.plot(x_fit,y_fit, color='red')
# popt = [0.04,0.005,1,0.07,0.005,0.2,0]
# y_fit = gauss(x_fit, *popt)
# plt.plot(x_fit,y_fit)

epsilon=0.99
x_fit_1 = np.linspace(x[1006-i_min],x[1024-i_min],num=1000)
x_fit_2 = np.linspace(x[995-i_min],x[1006-i_min],num=1000)
y_fit_1 = gauss(x_fit_1, *popt)
y_fit_2 = gauss(x_fit_2, *popt)
a_x_0, a_x_1=halbwertsbreite(popt[0], x_fit_1,y_fit_1,popt)
#plt.plot([a_x_0,a_x_0], [0,80])
#plt.plot([a_x_1,a_x_1], [0,80])
b_x_0, b_x_1=halbwertsbreite(popt[3], x_fit_2,y_fit_2,popt)
#plt.plot([b_x_0,b_x_0], [0,80])
#plt.plot([b_x_1,b_x_1], [0,80])

popt_error=np.sqrt(np.diag(pcov))
plt.plot(popt[0],gauss(popt[0], *popt), color='green', marker='+', ms = 10, label='$\\alpha_{H_\\alpha,0}$=(%.5f $\pm$ %.5f)$^\circ$\n$\Delta\\alpha=(%.5f\pm%.5f)^\circ$'%(popt[0],popt_error[0],a_x_0-a_x_1, (a_x_0-a_x_1)*0.01))
plt.plot(popt[3],gauss(popt[3], *popt), color='black', marker='+', ms = 10, label='$\\alpha_{H_\\alpha,1}$=(%.5f $\pm$ %.5f)$^\circ$\n$\Delta\\alpha=(%.5f\pm%.5f)^\circ$'%(popt[3],popt_error[3],b_x_0-b_x_1, (b_x_0-b_x_1)*0.01))
plt.legend()
...
###
# Berechne Auflösungsvermögen des Gitters
###
print('\n Auflösungsvermögen')
N = 25*10**(-3)/d
print(d)
